...
// vector of pointers to Conn structs
// fd2conn is a map of fd to connection?
...
static bool try_one_request(Conn *conn) {
    ...
    // IDK how this removes the request from the buffer
...
int main() {
    ...
    while (true) {
        ...
        for (size_t i = 1; i < poll_args.size(); ++i) {
            if (poll_args[i].revents) {
                ...
                if (conn->state == STATE_END) {
                    // client closed normally, or something bad happened lol
        ...
        // try to accept a new conn if the listening fd is active
